...
func TestCommandHelp(t *testing.T) {
	tests := []struct {
		...
		{[]string{"service", "--help"}, []string{"Manage persistent", "update", "restart", "remove"}},
		{[]string{"service", "add", "--help"}, []string{"godo service add", "--env-file", "--no-agent", "arguments"}},
		{[]string{"service", "restart", "--help"}, []string{"without rebuilding", "environment file"}},
		{[]string{"service", "edit", "--help"}, []string{"godo service edit", "--workdir", "--agent", "empty list clears"}},
